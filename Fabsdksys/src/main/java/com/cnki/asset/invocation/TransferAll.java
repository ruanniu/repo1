...
public class TransferAll {
	...
	public static void main(String args[]) {
		try {
			...
			for (ProposalResponse res: responses) {
				...
				Logger.getLogger(InvokeChaincode.class.getName()).log(Level.INFO,"Invoked transferAll on "+Config.CHAINCODE_1_NAME + ". Status - " + status);
	...
	private static TransferProposal createTransferProposal() {
		...
		//1、先设置交易权利描述：基本信息
		transactionRights.setPkri("Pkri1001");//确权标识
		transactionRights.setVersion("1.0");//设置版本
		transactionRights.setTransactionType("原始全部转让"); //设置交易类型：原始全部转让
		...
		proposal.setTrtype(transactionRights.getTransactionType());//设置交易类型为：原始全部转让
		proposal.setPkri(transactionRights.getPkri()); //设置确权标识
		proposal.setFpkrai("pkrai101"); // 设置发布资产标识：此处为资产登记的资产标识pkrai101
		proposal.setTpkrai("pkrai101_tran0"); //生成资产标识
		proposal.setTpkrai1("pkrai101_tran1"); //生成资产标识
