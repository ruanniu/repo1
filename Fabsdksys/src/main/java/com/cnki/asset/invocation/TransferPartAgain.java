...
//部分权利再转让交易
public class TransferPartAgain {
	...
	public static void main(String args[]) {
		try {
			...
			request.setFcn("transferPartAgain");
	...
	private static TransferProposal createTransferProposal() {
		...
		//1、先设置交易权利描述：基本信息
		transactionRights.setPkri("Pkri1002_again");//确权标识
		transactionRights.setVersion("1.0");//设置版本
		transactionRights.setTransactionType("部分再转让"); //设置交易类型：部分再转让
		...
		//转让部分权利项：实际转让的权利项有应用层传递，此处示例转让前5个权利项
		for(int i=0;i<5;i++){
		...
		proposal.setTpkrai("pkrai101_tran1_again"); //生成资产标识
		proposal.setTpkrai1("pkrai101_tran1_again1"); //生成资产标识
