...
public class DeployInstantiateChaincode {
	...
	public static void main(String[] args) {
		try {
			...
			//通过org1的管理员用户，根据通道名，获取通道，并把该通道相关的节点都加进来
			...
			//fabric客户端绑定到组织2的管理员用户：相当于cli容器链接到org2的peer0节点
			fabClient.getInstance().setUserContext(org2Admin);
			
			//对org2的节点：安装链码，并显示响应结果
			...
			//命令：peer chaincode instantiate -o orderer.example.com:7050 -C mychannel -n mycc  -v 1.0 -c '{"Args":[]}' -P "AND ('Org1MSP.peer','Org2MSP.peer')"
			//构建通道客户端：用来发送该通道的链码实例化请求
